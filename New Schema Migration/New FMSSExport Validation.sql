SELECT * FROM FMSSEXPORT2
-- Standard Requirements (pg 10)
-- FACLOCID
SELECT Location, Count(*) FROM FMSSEXPORT2 group by Location order by location
-- UNITCODE
SELECT Park, Count(*) FROM FMSSEXPORT2 group by Park order by Park
SELECT Park, Count(*) FROM FMSSEXPORT2 where Park not in (select Code from DOM_UNITCODE) group by Park order by Park
-- REGION
SELECT Region, Count(*) FROM FMSSEXPORT2 group by Region
-- FACOCCUPANT
SELECT Occupant, Count(*) FROM FMSSEXPORT2 group by Occupant
SELECT Occupant, Count(*) FROM FMSSEXPORT2 where Occupant not in (select Code from DOM_FACOCCUMAINT) group by Occupant
-- LCSID
SELECT CLASSSTR, Count(*) FROM FMSSEXPORT2 group by CLASSSTR order by CLASSSTR
SELECT CLASSSTR, Count(*) FROM FMSSEXPORT2 where CLASSSTR not in ('', 'NONE', 'N', 'NA', 'N/A', 'LCS') group by CLASSSTR order by CLASSSTR
-- FACOWNER
SELECT Asset_Ownership, Count(*) FROM FMSSEXPORT2 group by Asset_Ownership order by Asset_Ownership
SELECT Asset_Ownership, Count(*) FROM FMSSEXPORT2 where Asset_Ownership not in (select Code from DOM_FACOWNER) group by Asset_Ownership order by Asset_Ownership
-- CLIID
SELECT CLINO, Count(*) FROM FMSSEXPORT2 group by CLINO order by CLINO
SELECT CLINO, Count(*) FROM FMSSEXPORT2 where CLINO not in ('', 'NONE', 'N', 'NA', 'N/A') group by CLINO order by CLINO
-- FACMAINTAIN
SELECT FAMARESP, Count(*) FROM FMSSEXPORT2 group by FAMARESP order by FAMARESP
SELECT FAMARESP, Count(*) FROM FMSSEXPORT2  where FAMARESP not in (select Code from DOM_FACOCCUMAINT) group by FAMARESP
SELECT case when FAMARESP = 'Fed Gov' then 'FEDERAL' when FAMARESP = 'State Gov' then 'STATE'  when FAMARESP = '' then NULL else upper(FAMARESP) end as FAMARESP, count(*) FROM FMSSEXPORT2 group by FAMARESP order by FAMARESP
-- SEASONAL
SELECT OPSEAS, Count(*) FROM FMSSEXPORT2 group by OPSEAS order by OPSEAS
SELECT case when OPSEAS = 'Y' then 'Yes' when OPSEAS = 'N' then 'No' else 'Unknown' end as OPSEAS, count(*) FROM FMSSEXPORT2 group by OPSEAS order by OPSEAS
-- FACUSE
SELECT PRIMUSE, Count(*) FROM FMSSEXPORT2 group by PRIMUSE order by PRIMUSE
SELECT PRIMUSE, Count(*) FROM FMSSEXPORT2 where PRIMUSE not in (select Code from DOM_FACUSE) group by PRIMUSE order by PRIMUSE
-- BLDGCODE
SELECT DOI_Code, Count(*) FROM FMSSEXPORT2 group by DOI_Code order by doi_code
SELECT DOI_Code, Count(*) FROM FMSSEXPORT2 where DOI_Code is not null and DOI_Code <> '00000000' group by DOI_Code order by doi_code
SELECT DOI_Code, Count(*) FROM FMSSEXPORT2 where DOI_Code in (select Code from DOM_BLDGCODE) group by DOI_Code order by doi_code
SELECT DOI_Code, Count(*) FROM FMSSEXPORT2 where DOI_Code not in (select Code from DOM_BLDGCODE) group by DOI_Code order by doi_code
-- Get Asset List for validation of AssetID
-- Extras
-- BLDGTYPE
SELECT Predominant_Use, Count(*) FROM FMSSEXPORT2 group by Predominant_Use order by Predominant_Use
SELECT Predominant_Use, Count(*) FROM FMSSEXPORT2 where Predominant_Use in (select Code from DOM_BLDGTYPE) group by Predominant_Use order by Predominant_Use
SELECT Predominant_Use, Count(*) FROM FMSSEXPORT2 where Predominant_Use not in (select Code from DOM_BLDGTYPE) group by Predominant_Use order by Predominant_Use
SELECT DOI_Code, Predominant_Use, Count(*) FROM FMSSEXPORT2 where Predominant_Use not in (select Code from DOM_BLDGTYPE) group by DOI_Code, Predominant_Use order by Predominant_Use
-- BLDGSTATUS
SELECT Status, Count(*) FROM FMSSEXPORT2 group by Status
SELECT Status, Count(*) FROM FMSSEXPORT2 where Status not in (select Code from DOM_BLDGSTATUS) group by Status
-- PARKID
SELECT PARKNUMB, Count(*) FROM FMSSEXPORT2 where PARKNUMB not in ('', 'n', 'n/a', 'na', 'none') group by PARKNUMB
...
-- UNITCODE
select p.OBJECTID, 'Error: UNITCODE does not match FMSS.Park' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- FACOCCUPANT
select p.OBJECTID, 'Error: FACOCCUPANT does not match FMSS.Occupant' as Issue from AKR_BLDG_CENTER_PT as p join
...
select p.OBJECTID, 'Error: LCSID does not match FMSS.CLASSSTR' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- FACOWNER (1284)
select p.OBJECTID, 'Error: FACOWNER does not match FMSS.Asset_Ownership' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- CLIID (0)
select p.OBJECTID, 'Error: CLIID does not match FMSS.CLINO' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- FACMAINTAIN (1378)
select p.OBJECTID, 'Error: FACMAINTAIN does not match FMSS.FAMARESP' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- SEASONAL (0)
select p.OBJECTID, 'Error: SEASONAL does not match FMSS.OPSEAS' as Issue from AKR_BLDG_CENTER_PT as p join
...
select p.OBJECTID, 'Error: FACUSE does not match FMSS.PRIMUSE' as Issue from AKR_BLDG_CENTER_PT as p join
...
select p.OBJECTID, 'Error: BLDGCODE does not match FMSS.DOI_Code' as Issue from AKR_BLDG_CENTER_PT as p join
...
-- BLDGSTATUS (143)
select p.OBJECTID, 'Error: BLDGSTATUS does not match FMSS.Status' as Issue from AKR_BLDG_CENTER_PT as p join
...
select p.OBJECTID, 'Error: PARKBLDGID does not match FMSS.PARKNUMB' as Issue from AKR_BLDG_CENTER_PT as p join
...
update p set FACOCCUPANT = f.Occupant from AKR_BLDG_CENTER_PT as p join
  ...
  on f.Location = p.FACLOCID where p.FACOCCUPANT <> f.Occupant
-- LCSID (61, largely leading zeros)
select p.LCSID, f.CLASSSTR from AKR_BLDG_CENTER_PT as p join
  (select CLASSSTR, Location FROM FMSSEXPORT2 where CLASSSTR not in ('', 'NONE', 'N', 'NA', 'N/A', 'LCS')) as f
...
update p set CLIID = f.CLINO from AKR_BLDG_CENTER_PT as p join
  (select CLINO, Location FROM FMSSEXPORT2 where CLINO not in ('', 'NONE', 'N', 'NA', 'N/A')) as f
  on f.Location = p.FACLOCID where p.CLIID <> f.CLINO
-- FACMAINTAIN (1378)
...
update p set SEASONAL = f.OPSEAS from AKR_BLDG_CENTER_PT as p join
  (SELECT case when OPSEAS = 'Y' then 'Yes' when OPSEAS = 'N' then 'No' else 'Unknown' end as OPSEAS, location FROM FMSSEXPORT2) as f
  on f.Location = p.FACLOCID where p.SEASONAL <> f.OPSEAS
-- FACUSE (21 - investigate)
select p.FACUSE, f.PRIMUSE from AKR_BLDG_CENTER_PT as p join
  (SELECT PRIMUSE, Location FROM FMSSEXPORT2 where PRIMUSE in (select Code from DOM_FACUSE)) as f
...
update p set FACUSE = f.PRIMUSE from AKR_BLDG_CENTER_PT as p join
  ...
  on f.Location = p.FACLOCID where p.FACUSE <> f.PRIMUSE
-- BLDGCODE (10 - investigate)
select p.BLDGCODE, f.DOI_Code from AKR_BLDG_CENTER_PT as p join
  (SELECT DOI_Code, Location FROM FMSSEXPORT2 where DOI_Code in (select Code from DOM_BLDGCODE)) as f
...
-- BLDGTYPE (Ignore values in FMSS; we will calc from BLDGCODE)
-- BLDGSTATUS (143)
select p.BLDGSTATUS, f.Status from AKR_BLDG_CENTER_PT as p join
  (SELECT Status, Location FROM FMSSEXPORT2 where Status in (select Code from DOM_BLDGSTATUS)) as f
...
update p set BLDGSTATUS = f.Status from AKR_BLDG_CENTER_PT as p join
  ...
  on f.Location = p.FACLOCID where p.BLDGSTATUS <> f.Status
-- PARKBLDGID (145 - investigate)
select p.PARKBLDGID, f.PARKNUMB from AKR_BLDG_CENTER_PT as p join
  (SELECT PARKNUMB, Location FROM FMSSEXPORT2 where PARKNUMB not in ('', 'n', 'n/a', 'na', 'none')) as f
...
-- UNITCODE (nothing)
select p.UNITCODE, f.Park from AKR_BLDG_CENTER_PT as p join
  (SELECT Park, Location FROM FMSSEXPORT2 where Park in (select Code from DOM_UNITCODE)) as f
  on f.Location = p.FACLOCID where (p.UNITCODE is null and f.Park is not null) order by p.UNITCODE, f.Park
update p set UNITCODE = f.Park from AKR_BLDG_CENTER_PT as p join
  (SELECT Park, Location FROM FMSSEXPORT2 where Park in (select Code from DOM_UNITCODE)) as f
