...
def find_bldg_without_points(point_type):
    ...
    missing = find_missing_links(constants.bldg_table, temp_table,
...
def links_without_bldg():
    return find_missing_links(constants.link_table, constants.bldg_table,
                              constants.bldg_pk_name)


def links_without_fmss():
    links = find_missing_links(constants.link_table, constants.fmss_table,
                               constants.fmss_fk_name, constants.fmss_pk_name)
...
def links_without_lcs():
    links = find_missing_links(constants.link_table, constants.lcs_table,
                               constants.lcs_pk_name)
...
def bldgs_without_fmss():
    ...
    if items:
        ...
        for i in items:
            ...
            with arcpy.da.SearchCursor(table1, ['Unit_Code', 'Common_Name'],
                                       where1) as cursor1, \
                    arcpy.da.SearchCursor(table2, ['Is_Extant'],
                                          where2) as cursor2:
                is_extant = ''
...
def fmss_without_link():
    items = find_missing_links(constants.fmss_table, constants.link_table,
                               constants.fmss_pk_name, constants.fmss_fk_name)
    if items:
...
def polys_without_bldg():
    return find_missing_links(constants.poly_fc, constants.bldg_table,
                              constants.bldg_pk_name)


def points_without_bldg():
    return find_missing_links(constants.point_fc, constants.bldg_table,
                              constants.bldg_pk_name)
...
operations = [
    ("Duplicate Building Ids in the Buildings Table", find_dup_bldgs,
     "Building_ID", None),
    ...
    ("Buildings with UnitCode and No FMSS_ID in Links Table",
     bldgs_without_fmss, ['Park', 'Status', 'Description', 'ID', 'Type'],
     constants.no_fmss_export),
    ("FMSS Records without a Record in the Link Table", fmss_without_link,
